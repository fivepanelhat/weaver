...
try:
    ...
    # Minimal fallback splitter with similar interface for environments without the package
    class RecursiveCharacterTextSplitter:
        ...
        def split_text(self, text: str) -> List[str]:
            # naive splitting: break on double-newlines into paragraphs, then further into fixed-size windows
...
class AgnosticDataIngestionEngine:
    ...
    def __init__(self, chunk_size: int = 500, chunk_overlap: int = 50):
        # We set a sensible overlap so context isn't lost at the boundaries of the chunks.
    ...
    def chunk_and_tag_document(self, raw_text: str, tenant_id: str, source_id: str) -> List[Dict[str, Any]]:
        """
        ...
        for index, chunk_text in enumerate(raw_chunks):
            # Generate a unique cryptographic ID for this specific paragraph/chunk
            chunk_id = str(uuid.uuid4())

            # Assemble the exact schema we mapped out for the PostgreSQL/pgvector table
            chunk_record = {
                "chunk_id": chunk_id,
                "source_id": source_id,
                "tenant_id": tenant_id,  # The absolute non-negotiable multi-tenant boundary
                "content_payload": chunk_text,
                # Metadata is useful for the AI to know where it is in the document
                "metadata": {
                    "chunk_index": index,
        ...
        print(f"Processed {len(processed_chunks)} chunks for Tenant: {tenant_id}")

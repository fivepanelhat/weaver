...
def agnostic_intake_node(state: HelpdeskState, *args, **kwargs):
    ...
    context_str = (
        "Simulated retrieval: Custom cut piping is strictly non-refundable."
    )
    ...
    if (
        "angry" in state["user_message"].lower()
        or "refund" in state["user_message"].lower()
    ):
        next_step = "escalation"
        response_text = (
            "I understand you need a refund. Let me get a human manager."
        )
    else:
        next_step = "fulfilment"
        response_text = (
            "Let me check the status of your order based on our policies."
        )

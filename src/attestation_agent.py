...
def generate_tpm_quote(nonce_hex):
    ...
    try:
        ...
        # We check PCR bank sha256, registers 0 through 7 (Core Boot Integrity chain)
        tpm_cmd = [
            "tpm2_quote",
            "-c", "0x81010002",      # The persistent handle index of your Attestation Identity Key (AIK)
            "-l", "sha256:0,1,2,3,4,5,6,7", 
            "-q", nonce_bin_path,    # The challenge injection
            "-m", quote_path,        # Output structural attest file
            "-s", sig_path,          # Output hardware cryptographic signature
            "-p", pcr_path,          # Output targeted PCR state values
            "-g", "sha256"
        ]
        
        subprocess.run(tpm_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        # 3. Package the binary artifacts into hexadecimal transport payloads
        with open(quote_path, "rb") as q, open(sig_path, "rb") as s, open(pcr_path, "rb") as p:
            attestation_payload = {
                "quote": q.read().hex(),
        ...
        print(f"[CRITICAL HARDWARE FAULT] TPM execution failed: {e.stderr.decode()}")
        return None
    finally:

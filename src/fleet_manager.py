...
def push_fleet_policy(target_group, min_firmware_ver, allow_actuation):
    ...
    client.publish(POLICY_TOPIC, json.dumps(signed_envelope), qos=2, retain=True)
    print(f"[FLEET CONTROL] Pushed signed security policy to group: {target_group}")
    client.disconnect()
...
def on_heartbeat_received(client, userdata, msg):
    try:
        ...
        print(f"[FLEET MONITOR] Node {node_id} reported in. Status: {payload.get('posture')}")
